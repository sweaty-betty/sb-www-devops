...
Describe "Switch-SqlDatabases unit tests" -Tag "Unit" {
    ...
    BeforeAll {
        ...
        Mock Get-AzureRmSqlDatabase {
            $mock = '{ "ResourceGroupName": "sb-foo-bar-rg", "ServerName": "sb-foo-bar-sql", "DatabaseName": "sb-foo-bar-db", "Location": "westeurope" }'
    ...
    It "Should do a single check and a single rename if the existing database does not exist" {
        ...
        .\Switch-SqlDatabases -ResourceGroupName sb-foo-bar-rg -SQLServerName sb-foo-bar-sql -ExistingDatabaseName foobar-db -ReplacementDatabaseName foobar-new
