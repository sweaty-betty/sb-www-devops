...
Describe "Get-AppServiceAppSetting unit tests" -Tag "Unit" {
    ...
    It "Should error app setting does not exist" {
        ...
        .\Get-AppServiceAppSetting -ResourceGroupName sb-foo-bar-rg -AppServiceName sb-foo-bar-as -AppSetting notasetting
    ...
    It "Should run Invoke-Sqlcmd with inputfile when valid script is passed" {
        ...
        $output = .\Get-AppServiceAppSetting -ResourceGroupName sb-foo-bar-rg -AppServiceName sb-foo-bar-as -AppSetting foo
