...
Describe "New-KeyVault unit tests" -Tag "Unit" {
    ...
    BeforeAll {
        Mock Get-AzureRmResourceGroup { return ConvertFrom-Json '{ "ResourceGroupName": "sb-foobar-rg", "Location": "westeurope" }' }
        Mock New-AzureRmKeyVault { return ConvertFrom-Json '{ "VaultName": "sb-foobar-kv", "AccessPolicies": [ { "ObjectId": "12345678-abcd-1234-5678-1234567890ab" } ] }' }
    ...
    It "Should not create anything if the key vault already exist" {
        Mock Get-AzureRmKeyVault { return ConvertFrom-Json '{ "VaultName": "sb-foobar-kv", "ResourceGroupName": "sb-foobar-rg", "Location": "westeurope" }' }
