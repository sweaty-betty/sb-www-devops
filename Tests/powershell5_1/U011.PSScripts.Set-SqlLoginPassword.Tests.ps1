...
Describe "Set-SqlLoginPassword unit tests" -Tag "Unit" {
    ...
    BeforeEach {
        ...
        $params = @{
            ResourceGroupName = "sb-foo-bar-rg"
            SQLServerName     = "sb-foo-bar-sql"
            SQLDatabase       = "sb-foo-bar-db" 
        ...
        Mock Get-AzureRmSqlServer {
            $mock = '{ "ResourceGroupName": "sb-foo-bar-rg", "ServerName": "sb-foo-bar-sql", "FullyQualifiedDomainName": "sb-foo-bar-sql.database.windows.net" }'
            return ConvertFrom-Json $mock
        }
    
        # mock Get-AzureRmSqlDatabase returns offline
        Mock Get-AzureRmSqlDatabase {
            $offlinemock = '{ "ResourceGroupName": "sb-foo-bar-rg", "ServerName": "sb-foo-bar-sql", "DatabaseName": "sb-foo-bar-db", "Status": "Offline" }'
    ...
    It "Should issue a single Invoke-Sqlcmd when just reseting password" {
        ...
        Mock Get-AzureRmSqlDatabase {
            $onlinemock = '{ "ResourceGroupName": "sb-foo-bar-rg", "ServerName": "sb-foo-bar-sql", "DatabaseName": "sb-foo-bar-db", "Status": "Online" }'
