...
Describe "New-ApplicationRegistration unit tests" -Tag "Unit" {
    ...
    It "Should throw an error if a secret is requested and the KeyVault doesn't exist" {
        ...
        $CmdletParameters = @{
            AppRegistrationName = "sb-foo-bar-app"
            AddSecret           = $true
            KeyVaultName        = "sb-foo-shared-kv"
        }

        { .\New-ApplicationRegistration @CmdletParameters } | Should -Throw "KeyVault sb-foo-shared-kv doesn't exist, nowhere to store secret"
    ...
    It "Should throw an error if a secret is requested and Azure DevOps SPN doesn't have SET permissions on the KeyVault" {
        ...
        { .\New-ApplicationRegistration @CmdletParameters } | Should -Throw "Service Principal 4a11d94c-9c97-4c0b-8f85-476c1ef15956 doesn't have Set permission on KeyVault sb-foo-shared-kv"
